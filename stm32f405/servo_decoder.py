...
class ServoDecoder:
    ...
    def __init__(self, servo_pin, timer_number, channel_number):
        self.timer = pyb.Timer(timer_number, prescaler=83, period=0x0fffffff)
        self.ic_pin = servo_pin
        self.ic_pin.init(Pin.AF_PP, pull=Pin.PULL_NONE, alt=Pin.AF1_TIM2)
        self.ic = self.timer.channel(channel_number, pyb.Timer.IC, pin=self.ic_pin, polarity=pyb.Timer.BOTH)
        self.ic_start = 0
        self.pulse_width = 0
